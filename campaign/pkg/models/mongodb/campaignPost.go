...
type CampaignPostModel struct {
	C *mongo.Collection
}

// All method will be used to get all records from the movies table.
func (m *CampaignPostModel) All() ([]models.CampaignPost, error) {
	// Define variables
	ctx := context.TODO()
	mm := []models.CampaignPost{}
...
func (m *CampaignPostModel) FindByID(id string) (*models.CampaignPost, error) {
...
func (m *CampaignPostModel) Insert(movie models.CampaignPost) (*mongo.InsertOneResult, error) {
	return m.C.InsertOne(context.TODO(), movie)
}

// Delete will be used to delete a movie registry
func (m *CampaignPostModel) Delete(id string) (*mongo.DeleteResult, error) {
	p, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		return nil, err
	}
